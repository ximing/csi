...
产出 `release/csi-extension-v0.4.0.zip`。自检:`unzip -l` 确认 zip 根有 `manifest.json`,`icons/` `_locales/` `background.js` `popup.html` `options.html` 齐全。
...
- [ ] Dashboard → **New Item** → 上传 `release/csi-extension-v0.4.0.zip`。
