...
- `skills/csi/`、`skills/csi-e2e/` — 随安装器分发到 `~/.claude/skills/` 的 Claude Code 技能

完整说明见 `README.md`；线上协议契约见 `docs/protocol.md`。

## 全局规则

- **协议先行**：`docs/protocol.md` 是 daemon 与 extension 两侧实现的唯一契约。任何协议变更先改该文件，再改实现。跨侧同步的完整要求见 `.claude/rules/protocol-sync.md`。
...
横切规则（按需加载）：`.claude/rules/protocol-sync.md`（`.catpaw/rules/` 下有同正文的 CatPaw 版本）。
